...
const tokenASwapAmount = 100, tokenBSwapAmount = 200, mintAmount = 1000;
...
describe("Swapper", function () {
    ...
    describe('create proposal', function () {
        it('should allow an arbitrary address to create a proposal', async () => {
            const tx = await swapper.connect(proposer).propose(holderA.address, tokenA.address, tokenASwapAmount, holderB.address, tokenB.address, tokenBSwapAmount, 0, 0);
    ...
    describe('cancel proposal', function () {
        beforeEach(async () => {
            await swapper.connect(proposer).propose(holderA.address, tokenA.address, tokenASwapAmount, holderB.address, tokenB.address, tokenBSwapAmount, 0, 4);
        })
    ...
    describe('approve proposal', function () {
        ...
        it('should allow one of the parties to approve the proposal', async () => {
            ...
            await expect(swapper.connect(holderA).approve(0))
                .to.emit(swapper, "DealApproved")
                .withArgs(0, holderA.address);
